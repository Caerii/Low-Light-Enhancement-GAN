...
for epoch in range(num_epochs):
    ...
    for i, (low_res, high_res) in enumerate(train_dataloader):
        ...
        # Calculate the total loss using the loss_fn
        total_loss, mae_loss, gan_loss = loss_fn(high_res, generated_imgs, real_labels, fake_labels, beta)
        ...
        # Save and display the input and output images every 10 batches
        if (i+1) % 10 == 0:
            for j, (input_img, output_img) in enumerate(zip(low_res, generated_imgs)):
                input_img = input_img.detach().cpu()
                output_img = output_img.detach().cpu()

                # Save input and output images
                input_img_path = os.path.join(output_dir, f"input_epoch{epoch}_batch{i+1}_img{j}.png")
                output_img_path = os.path.join(output_dir, f"output_epoch{epoch}_batch{i+1}_img{j}.png")
                save_image(input_img, input_img_path)
