...
def calculate_ssim(output, target):
    ...
    for i in range(output.size(0)):
        output_np = np.array(to_pil_image(output[i]))
        target_np = np.array(to_pil_image(target[i]))
        ssim_value = structural_similarity(output_np, target_np, multichannel=True)
        ssim_values.append(ssim_value)

    ssim = torch.tensor(ssim_values).mean()  # Calculate mean SSIM across the batch
    return ssim
...
for epoch in range(num_epochs):
    ...
    for i, (low_res, high_res) in enumerate(train_dataloader):
        ...
        # Save the generated images every 100 batches
        if (i+1) % 100 == 0:
    ...
    fig, axes = plt.subplots(len(low_res), 2, figsize=(10, 20))
    for i in range(len(low_res)):
        # Plot input image
        axes[i, 0].imshow(low_res[i].permute(1, 2, 0).cpu().detach())
        axes[i, 0].set_title("Input Image")
        axes[i, 0].axis("off")

        # Plot output image
        axes[i, 1].imshow(generated_imgs[i].permute(1, 2, 0).cpu().detach())
        axes[i, 1].set_title("Output Image")
        axes[i, 1].axis("off")

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"input_output_epoch{epoch}.png"))
    plt.close()
