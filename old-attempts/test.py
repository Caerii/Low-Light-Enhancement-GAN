...
class EDSR(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, num_features=3, num_blocks=16, res_scale=0.1):
        super(EDSR, self).__init__()
        self.conv_input = nn.Conv2d(in_channels, num_features, kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.res_blocks = nn.ModuleList([ResidualBlock(num_features, res_scale) for _ in range(num_blocks)])
        self.conv_residual = nn.Conv2d(in_channels, num_features, kernel_size=1, stride=1, padding=0)  # 1x1 convolution for residual connection
        self.conv_output = nn.Conv2d(num_features, out_channels, kernel_size=3, stride=1, padding=1)

    def forward(self, x):
        out = self.relu(self.conv_input(x))
        residual = self.conv_residual(x)  # Apply 1x1 convolution to match the number of channels
    ...
    for i, (low_res, high_res) in enumerate(train_dataloader):
        ...
        if (i+1) % 10 == 0:
            for j, img in enumerate(generated_imgs):
                ...
                # Transpose the numpy array to correct the color channel ordering
                ...
                # Save the image
                img_pil.save(os.path.join(output_dir, f"generated_img_epoch{epoch}_batch{i+1}_img{j}.png"))
